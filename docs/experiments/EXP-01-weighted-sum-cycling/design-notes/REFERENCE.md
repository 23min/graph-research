...
superseded_by: E-EVOLVE-v2-methodology (see ADR 0001)
---

# E-EVOLVE-layout-pipeline — Reference Snapshot

This epic was frozen on 2026-04-17 rather than merged. The decision to pivot is recorded in `docs/decisions/0001-pivot-from-weighted-sum-single-corpus-ga-to-multi-objective-methodology.md`. The branch and tag are preserved so the work remains retrievable and browsable. Nothing from this branch was merged to `main` at freeze time.
...
- **Mode 2.5 (process bezier)** — same layout, bezier routing + metro-style dot stations. See `docs/experiments/EXP-01-weighted-sum-cycling/design-notes/mode-2.5-bezier-process.md` for the full design note.
...
Printed reference captures from the M-EVOLVE-01 freeze. A2 landscape PDFs, vector, white background, suitable for physical printing. The four archival files were originally committed on the upstream tag `reference/m-evolve-01`; they are preserved on the `research/v2-methodology` branch under the path below. The tag itself (as it exists in this repo after the filter-repo import) does not carry them — `work/reports/` was outside the import's path allowlist.

| Artifact | Purpose |
|----------|---------|
| `reports/showcase-m-evolve-01.pdf` | 5×5 matrix: metro-ref × process-default × process-ga × process-bezier × flow-legacy, on Stockholm, O2C, Incident Process, Dense Interchange, Parallel Processes. |
| `reports/showcase-m-evolve-01.html` | HTML sidecar with embedded SVGs (for future inspection). |
| `reports/stockholm-bezier-m-evolve-01.pdf` | Solo Stockholm × Process Bezier, A2 1-up, `labelSize: 18 × scale: 2.2`. |
| `reports/stockholm-bezier-m-evolve-01.html` | HTML sidecar. |

Selection files `reports/showcase-selection.mjs` and `reports/stockholm-bezier-selection.mjs` live alongside the archival artefacts. They are live configuration captured at freeze time, not archival snapshots.

All paths above are relative to this file's directory (`docs/experiments/EXP-01-weighted-sum-cycling/design-notes/`).

## Pivot rationale (short)

See `docs/decisions/0001-pivot-from-weighted-sum-single-corpus-ga-to-multi-objective-methodology.md` for the full record. Summary:
